...
const query = async (url, method, body, header) => {
  ...
  const urlApiTest = process.env.REACT_APP_API_URL
...
export const getFeaturedPosts = async () => await query("/posts?featured=true&limit=500", "GET", null)
...
export const getFeaturedUsers = async () => await query("/users/all/featured?limit=500", "GET", null)
...
export const getAllTags = async () => await query("/tags?limit=500", "GET", null)
...
// Uploads
const token = isAuthenticated() ? JSON.parse(localStorage.getItem("currentUser")).token : ""
export const uploadImage = async (formData) => await query("/upload", "POST", formData, {
  "Authorization": `Bearer ${token}`
})